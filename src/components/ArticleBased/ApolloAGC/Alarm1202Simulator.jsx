...
const STAGES = [
  {
    ...
    jobs: [
      { id: "nav",    name: "NAVIGATION",   priority: 1, color: "#ef4444", note: "Position + velocity — 25Hz update" },
      { id: "guide",  name: "GUIDANCE",     priority: 2, color: "#f97316", note: "P63 Braking — descent trajectory" },
      { id: "thrust", name: "THRUST VEC",   priority: 3, color: "#eab308", note: "Engine gimbal + throttle" },
      { id: "dsky",   name: "DSKY DISPLAY", priority: 4, color: "#22c55e", note: "Crew readout — altitude, velocity" },
      { id: "crew",   name: "CREW MONITOR", priority: 5, color: "#3b82f6", note: "System health checks" },
    ],
    droppedJobs: [],
    cpuLoad: 72,
    alarmCode: null,
  ...
  },
  {
    ...
    jobs: [
      ...
      { id: "dsky",   name: "DSKY DISPLAY", priority: 4, color: "#22c55e", note: "Crew readout — altitude, velocity" },
      { id: "crew",   name: "CREW MONITOR", priority: 5, color: "#3b82f6", note: "System health checks" },
      { id: "r1",     name: "RADAR IRQ",    priority: 7, color: "#ec4899", note: "Rendezvous radar interrupt — not needed!", spurious: true },
      { id: "r2",     name: "RADAR IRQ",    priority: 7, color: "#ec4899", note: "Rendezvous radar interrupt — not needed!", spurious: true },
    ],
    droppedJobs: [],
    cpuLoad: 88,
    alarmCode: null,
  ...
  },
  {
    ...
    description:
      "The EXEC's job table has exactly 8 slots. All are occupied. A new job arrives and finds no room. The Executive has no safe way to proceed — it cannot silently drop a job it doesn't know is unimportant. It raises alarm code 1202.",
    jobs: [
      { id: "nav",    name: "NAVIGATION",   priority: 1, color: "#ef4444", note: "Position + velocity — 25Hz update" },
      { id: "guide",  name: "GUIDANCE",     priority: 2, color: "#f97316", note: "P63 Braking — descent trajectory" },
      { id: "thrust", name: "THRUST VEC",   priority: 3, color: "#eab308", note: "Engine gimbal + throttle" },
      ...
      { id: "r1",     name: "RADAR IRQ",    priority: 7, color: "#ec4899", note: "Rendezvous radar interrupt — not needed!", spurious: true },
      { id: "r2",     name: "RADAR IRQ",    priority: 7, color: "#ec4899", note: "Rendezvous radar interrupt — not needed!", spurious: true },
      { id: "r3",     name: "RADAR IRQ",    priority: 7, color: "#ec4899", note: "Cannot be scheduled — no slot available", spurious: true, overflow: true },
    ],
    droppedJobs: [],
    cpuLoad: 100,
    alarmCode: "1202",
  ...
  },
  {
    ...
    jobs: [
      { id: "nav",    name: "NAVIGATION",   priority: 1, color: "#ef4444", note: "KEPT — navigation state preserved from protected memory" },
      { id: "guide",  name: "GUIDANCE",     priority: 2, color: "#f97316", note: "KEPT — trajectory program restarted" },
      { id: "thrust", name: "THRUST VEC",   priority: 3, color: "#eab308", note: "KEPT — engine still controlled" },
      { id: "dsky",   name: "DSKY DISPLAY", priority: 4, color: "#22c55e", note: "KEPT — alarm code shown to crew" },
    ],
    droppedJobs: [
  ...
  },
  {
    ...
    description:
      "The computer resumes normal guidance within 1.5 seconds of the restart. Flight controller Jack Garman — who had a handwritten crib sheet of alarm codes — tells Steve Bales: \"GO. We're GO.\" The radar keeps firing interrupts, but BAILOUT keeps executing the restart cycle fast enough that guidance is never lost.",
    jobs: [
      { id: "nav",    name: "NAVIGATION",   priority: 1, color: "#ef4444", note: "Position + velocity — resumed" },
      { id: "guide",  name: "GUIDANCE",     priority: 2, color: "#f97316", note: "P64 Approach — switched automatically" },
      { id: "thrust", name: "THRUST VEC",   priority: 3, color: "#eab308", note: "Engine gimbal + throttle" },
      { id: "dsky",   name: "DSKY DISPLAY", priority: 4, color: "#22c55e", note: "Crew readout restored" },
      { id: "crew",   name: "CREW MONITOR", priority: 5, color: "#3b82f6", note: "Health checks restored" },
    ],
    droppedJobs: [],
    cpuLoad: 72,
    alarmCode: null,
  ...
  if (!job) {
    return (
      <div className="flex items-center gap-2.5 px-3 py-2 rounded border border-dashed border-gray-200 dark:border-gray-700">
        <span className="text-[10px] font-mono text-gray-300 dark:text-gray-600 w-3 text-center">
          {index + 1}
        </span>
        <span className="text-[11px] font-mono text-gray-300 dark:text-gray-600 italic">
          empty
        </span>
      </div>
    );
  ...
  return (
    <div
      className={`flex items-start gap-2.5 px-3 py-2 rounded border transition-all duration-300 ${
        job.overflow
          ? "animate-pulse border-red-400 bg-red-50 dark:bg-red-950/30"
          : "border-gray-200 dark:border-gray-700/80"
      }`}
      style={
        !job.overflow
          ? { borderLeftColor: job.color, borderLeftWidth: 2 }
          : {}
      }
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 mb-0.5">
          <span
            className="text-[11px] font-mono font-medium"
            style={{ color: job.overflow ? "#ef4444" : job.color }}
          >
            {job.name}
          </span>
          ...
          {job.spurious && !job.overflow && (
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-pink-100 dark:bg-pink-950/50 text-pink-500 dark:text-pink-400">
              spurious
            </span>
          )}
          {job.overflow && (
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-red-100 dark:bg-red-950/50 text-red-500">
              NO SLOT
            </span>
          )}
        </div>
...
export default function Alarm1202Simulator() {
  ...
  // Pad job list to always show 8 slots
    ...
    <div className="my-10 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden bg-white dark:bg-gray-900 font-mono">
      ...
      <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60">
        <div className="flex items-center justify-between gap-3">
          ...
          {stage.alarmCode && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded border border-red-400 bg-red-50 dark:bg-red-950/30">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
              <span className="text-xs text-red-600 dark:text-red-400 tracking-widest uppercase">
      ...
      <div className="p-5 space-y-5">
        {/* Stage info */}
        <div>
          <p className="text-xs text-gray-800 dark:text-gray-200 font-medium mb-0.5">
            {stage.title}
          </p>
        ...
        {/* CPU Load */}
        <CpuBar load={stage.cpuLoad} alarm={stage.alarmCode} />
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {stage.droppedJobs.map((d, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2.5 px-3 py-2 rounded border border-dashed border-gray-200 dark:border-gray-700 opacity-50"
                >
                  <span className="text-gray-400 text-xs mt-0.5">✕</span>
                  <div>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 line-through">
                      {d.name}
                    </p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">
                      {d.reason}
